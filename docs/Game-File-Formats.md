...
> 因此游戏名称与描述在原机中本就是图像，无需文本渲染即可显示：名称来自名称横幅图，描述随预览截图图一并解码绘制。
...
> 预览与**烘焙在其中的描述文字**来自预览截图图。真正尚未解析的只是 `.dat` 头部那些
> 用途未知的字段（疑似配置参数），它们不影响菜单显示。
...
| `.dat` | ✅ 部分支持 | 前端菜单解码名称横幅与预览截图（描述已烘焙在预览图内）；头部配置字段未解析 |

### 当前不支持的格式

| 格式 | 支持状态 | 说明 |
|------|---------|------|
| `.dat` 头部配置字段 | ❌ 不支持 | 名称/预览/描述均已显示，仅头部用途未知的配置字段未解析（不影响菜单显示） |
