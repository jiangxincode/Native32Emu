...
| `.dat` | 83 | `INFO` 头二进制 | 游戏元数据、缩略图、描述 | ✅ 部分支持（前端菜单缩略图） |
...
> 游戏名称在原机中即为预渲染的图像横幅，因此通过解码名称横幅图即可显示，
> 无需文本渲染。名称横幅与预览截图都按与普通游戏画面**完全相同**的方式解码绘制
> （忠实保留原始颜色、透明像素跳过、不透明像素覆盖），不做额外处理。
>
> **注意：** `.dat` 横幅与预览均为奇数宽度的 4:2:0 图像。4:2:0 每行的 quad 数为
> `ceil(宽度/2)`，解码器必须用向上取整，否则每行会少消费一个 quad，导致图像逐行
> 横向漂移、整体被对角错切。
...
> **注意：** 缩略图/名称图以外的 `.dat` 内容（详细描述文本、配置参数）尚未解析。
> `FormateStr`（任意文本渲染，需要字体子系统）在 `FHUI` 中仅用于清空文本标签，
> 故暂未实现。
...
| `.dat` | ✅ 部分支持 | 前端菜单 `FHUI.smf` 读取其中的预览缩略图 |

### 当前不支持的格式

| 格式 | 支持状态 | 说明 |
|------|---------|------|
| `.dat`（描述/配置） | ❌ 不支持 | 仅解析缩略图；描述文本、配置参数未解析 |
| `.mpg` | ✅ 支持 | MPEG-1 过场视频播放 |
